...
def compute_state_stats(state, proc_date):
    '''
    ...
    for election in elections:
        ...
        if not data.time_series_has_date(cursor, proc_date, elec_dt):
            ...
            while proc_date_cur and proc_date_cur <= proc_date_end:
                ...
                proc_total = proc_ts[proc_idx]['count'] if proc_idx else prev_proc_tot
                proc_uniq = proc_ts[proc_idx]['diff'] if proc_idx else None

                rej_total = rej_ts[rej_idx]['count'] if rej_idx else prev_rej_tot
                rej_uniq = rej_ts[rej_idx]['diff'] if rej_idx else None

                cur_total = cur_ts[cur_idx]['count'] if cur_idx else prev_cur_tot
                cur_uniq = cur_ts[cur_idx]['diff'] if cur_idx else None

                if proc_total < 5000:
                    proc_total = prev_proc_tot + proc_total

                prev_proc_tot = proc_total
                prev_rej_tot = rej_total
                prev_cur_tot = cur_total
...
def compute_county_stats(state, proc_date):
    '''
    ...
    for election in elections:
        ...
        for entry in counties:
            ...
            if not data.time_series_has_date(cursor, proc_date, elec_dt, county):
                proc_ts = [] # temp for time-saving, don't really need proc for counties
                #proc_ts = elec.get_unique_processed()
                rej_ts = elec.get_unique_rej()
                cur_ts = elec.get_unique_cured()
...
def compute_2021_runoff_election():
    ...
    while proc_date <= end_date:
        compute_state_stats('GA', proc_date)
        compute_county_stats('GA', proc_date)
